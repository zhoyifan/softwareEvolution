...
def sum_up(intervals):
    ...
    for start,end in intervals:
        res+=end-start+1
    return res
# after this for loop, key of repeatPair does not change, value will be (version:number) dict.
pattern=r"(\S+)\:(\d+)\,(\d+)"
for versionPair, value in repeatPair.items():
    ...
    for version, repeats in value.items():
        ...
        repeatPair[versionPair][version]=versionRepeat
...
for versionPair, repeats in repeatPair.items():
    versionList=list(versionPair)
    matrix.loc[versionList[0],versionList[1]]=repeatPair[versionPair][versionList[0]]
    matrix.loc[versionList[1],versionList[0]]=repeatPair[versionPair][versionList[1]]
